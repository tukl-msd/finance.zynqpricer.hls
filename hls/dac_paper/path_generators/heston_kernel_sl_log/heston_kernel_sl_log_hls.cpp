...
#include "heston_kernel_sl_log_hls.hpp"
		...
		hls::stream<calc_t> &prices)
{
	...
	#pragma HLS interface ap_none port=inv_correlation
	#pragma HLS resource core=AXI4LiteS metadata="-bus_bundle params" variable=inv_correlation
...
//					l_state.barrier_hit = false;
				...
				calc_t w_vola = correlation * w_stock +
						inv_correlation * (calc_t) gaussian_rn2.read();
				...
				n_state.stock = l_state.stock + (double_riskless_rate - max_vola) *
						half_step_size + sqrt_step_size * sqrt_vola * w_stock;
				n_state.vola = l_state.vola + reversion_rate_TIMES_step_size *
						(long_term_avg_vola - max_vola) +
						vol_of_vol_TIMES_sqrt_step_size * sqrt_vola * w_vola;
