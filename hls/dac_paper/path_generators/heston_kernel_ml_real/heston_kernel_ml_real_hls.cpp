...
#include "heston_kernel_ml_real_hls.hpp"
...
state_t get_next_step(const params_ml params, const state_t l_state,
	...
	n_state.stock = l_state.stock * hls::expf(
			(params.double_riskless_rate - max_vola) *
...
void heston_kernel_ml_real(const params_ml params, hls::stream<calc_t> &gaussian_rn1,
			...
			for (ap_uint<6> j = 0; j != upper_j; ++j) {
				for (ap_uint<10> block_i = 0; block_i != BLOCK_SIZE;
					...
					if (is_fine) {
						...
						float w_vola = params.correlation * w_stock +
